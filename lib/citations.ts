...
export function parseResponse(provider:Provider,d:Obj,sites:Site[]){let answer='',searchUsed=false;const citations:Citation[]=[];const add=(a:Obj,text='')=>{const n=normalizeUrl(String(a.url||a.uri||''));if(!n)return;citations.push({url:n,title:String(a.title||''),text:text||String(a.cited_text||''),siteId:matchSite(n,sites)});};
 ...
 if(provider==='openai'){
  ...
  const c=d.candidates?.[0];if(!c)throw new Error('Gemini 답변이 없습니다. 차단 또는 연결 상태를 확인하세요.');if(c.finishReason&&c.finishReason!=='STOP')throw new Error('Gemini 답변이 중단되었습니다. 종료 코드: '+String(c.finishReason).replace(/[^A-Z_]/g,'').slice(0,80)+(c.finishReason==='MAX_TOKENS'?' · 답변 생성 토큰 한도에 도달했습니다.':c.finishReason==='SAFETY'?' · 안전 필터로 중단되었습니다.':c.finishReason==='LANGUAGE'?' · 지원 언어 제한으로 중단되었습니다.':''));answer=(c.content?.parts||[]).filter((p:Obj)=>!p.thought).map((p:Obj)=>p.text||'').join('');const meta=c.groundingMetadata;searchUsed=!!meta?.webSearchQueries?.length;for(const support of meta?.groundingSupports||[])for(const idx of support.groundingChunkIndices||[]){const web=meta.groundingChunks?.[idx]?.web;if(web)add(web,String(support.segment?.text||''));}
 }
